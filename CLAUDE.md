...
### 保留する（情報を入れる）
未来の予定を見つけたら（または既存記事で先の予定に触れたら）、`stock/upcoming.yml` にエントリを追加する。`status: pending`、`confidence`（confirmed/rumor）、`source` を明記し、`facts` には記事素材になる粒度で確定事実を書く。`publish_plan` に「いつ（配信日）・どの切り口で」記事化するかを記す。
